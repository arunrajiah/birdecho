...
- **App version shown in Settings** — a small footer at the bottom of the Settings tab now displays the installed version and build number (e.g. `BirdEcho v0.8.2 (23)`). Makes it easy to confirm which build you're running — useful after an in-app update, and a quick way to verify the version is reported correctly.
...
- **Favorites screen blank for BirdNET-Go/Pi users**: `app/(tabs)/favorites.tsx` imported `fetchSpecies` directly (BirdWeather-only) instead of going through `useApiAdapter()`. Starred species never loaded on non-BirdWeather connections. Fixed with adapter pattern.
- **Sighting detail screen failing for BirdNET-Go/Pi users**: `app/record/[id].tsx` imported `fetchRecord` directly (BirdWeather-only). The detail screen always errored for BirdNET-Go and BirdNET-Pi users. Fixed with adapter pattern.

These are the last two screens that bypassed the adapter layer. All app screens now route exclusively through `useApiAdapter()`.
...
- **Offline cache**: the last 24 hours of query data (detections, species, stats) are now persisted to `AsyncStorage` via `@tanstack/react-query-persist-client`. The feed, species list, and stats remain readable when the app opens without a network connection, and stale data is shown instantly while a background refresh runs. Data is refreshed in the foreground after 5 minutes of staleness; the on-disk cache expires after 24 hours. Writes are throttled to 2 s to avoid AsyncStorage churn.
...
- **BirdNET-Go `http://` connections still blocked on Android in v0.2.2** (issue #14): v0.2.2 set `usesCleartextTraffic` in the wrong location in `app.json` — Expo's prebuild silently ignores that field at `android.usesCleartextTraffic` and does not write it to `AndroidManifest.xml`. The correct approach is the `expo-build-properties` plugin, which is a proper config plugin that writes `android:usesCleartextTraffic="true"` into the generated manifest. Verified locally that the attribute is present in the manifest after prebuild.
